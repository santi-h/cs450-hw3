...
**************************************************************************************************/
int to_dns_style(const char* str_name, uint8_t* dns_name)
{
	int namelen = strlen( str_name);
	int part_len=0;
	int i;
	for(i=0;i<namelen;i++) {
		if(str_name[i]!='.') {
	...
	dns_name[namelen-part_len]=part_len;
	dns_name[namelen+1]=0;
	return namelen+2;
}
...
*	 
*	 Returns the number of bytes of dns_name read. dns_name+returnvalue = start of next field
*
*	<message> is the dns response
*	<dns_name> is where the dns_name field in <message> starts
*	<str_name> is the place to store the field
**************************************************************************************************/
int from_dns_style(uint8_t *message, uint8_t* dns_name, char* str_name) {
	...
	while(*dns_name) {
		if(part_remainder==0) {
			...
			if((*dns_name)>=0xc0) { 
				if(return_len==0) return_len = (dns_name-orig_name)+2;
